...
{
    public class TraineeCTC
    {
        public static void CTC_Master(string ctccodes, string filePath, string destinationFolder)
        {
            try
            {
                using (var p = new ExcelPackage(new FileInfo(ctccodes)))
                {
                    bool sheetExists = p.Workbook.Worksheets["Trainee"] != null;
                    if (sheetExists)
                    {
                        try
                        {
                            ...
                            using (var package = new ExcelPackage(new FileInfo(filePath)))
                            {
                                ...
                                using (var outputPackage = new ExcelPackage())
                                {
                                    ...
                                    {
                                        var ctcsheet = CTCPackage.Workbook.Worksheets["Trainee"];
                                    ...
                                    for (int row = 2; row <= lastRow2; row++)
                                    {
                                        ...
                                        bool trainee = Regex.IsMatch(joinerandchangessheet.Cells[row, designation].Text.ToLower(), $@"\b{Regex.Escape("trainee")}\b", RegexOptions.IgnoreCase);

                                        if (!string.IsNullOrEmpty(bgColor.Rgb) && !bgColor.Rgb.Equals("FFFFFF") && trainee)
                                        {
                                        ...
                                        else if (!string.IsNullOrEmpty(bgColor.Rgb) && !bgColor.Rgb.Equals("FFFFFF") && filePath.ToLower().Contains("celonis") && joinerandchangessheet.Cells[row, empgrade].Text.ToLower().Contains("trainee"))
                                        {
                                            HRID.Add(joinerandchangessheet.Cells[row, hrid2].GetValue<string>());
                                            PtLoc.Add(joinerandchangessheet.Cells[row, hrid2].GetValue<string>(), Service1.ShrinkString(joinerandchangessheet.Cells[row, ptloc].GetValue<string>()));
                                            Role.Add(joinerandchangessheet.Cells[row, hrid2].GetValue<string>(), Service1.ShrinkString(joinerandchangessheet.Cells[row, designation].GetValue<string>()));
                                    ...
                                    for (col = 2; col <= outputcolumns; col++)
                                    {
                                        ...
                                        if (outputWorksheet.Cells[2, col].Text.ToLower().Contains("metrononmetro"))
                                        {
                                            ...
                                            // Parse both formulas from metrononmetro[formula1][formula2]
                                            int firstOpen = formulaText.IndexOf('[');
                                            int firstClose = formulaText.IndexOf(']');
                                            int secondOpen = formulaText.IndexOf('[', firstClose + 1);
                                            int secondClose = formulaText.IndexOf(']', secondOpen + 1);

                                            string metroFormula = formulaText.Substring(firstOpen + 1, firstClose - firstOpen - 1);
                                            string nonMetroFormula = formulaText.Substring(secondOpen + 1, secondClose - secondOpen - 1);

                                            // Apply for each employee
                                            for (int row3 = 2; row3 <= outputRows; row3++)
                                            {
                                                id = outputWorksheet.Cells[row3, 1].Text.Trim();
                                                if (!PtLoc.TryGetValue(id, out location))
                                                    location = "NonMetro"; // Default
                                                location = Service1.ShrinkString(location);
                                                if (location.Contains("mumbai") || location.Contains("delhi") || location.Contains("kolkata") || location.Contains("chennai") || location.Contains("calcutta"))
                                                {
                                                    outputWorksheet.Cells[row3, col].Formula = AdjustFormulaToRow(metroFormula, 2, row3);
                                                }
                                                //outputWorksheet.Cells[row3, col].Formula = metroFormula.Replace("2", row3.ToString());
                                                else
                                                    outputWorksheet.Cells[row3, col].Formula = AdjustFormulaToRow(nonMetroFormula, 2, row3);
                                            }
                                        }
                                    ...
                                    string newFileName = Path.Combine(destinationFolder, Service1.FileCount + "]NEW_trainees_CTC" + Path.GetFileName(filePath));
