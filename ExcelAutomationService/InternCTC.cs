    ...
    public class InternCTC
    {
        public static void CTC_Master(string ctccodes, string filePath, string destinationFolder)
        {
            try
            {
                ...
                {
                    bool sheetExists = p.Workbook.Worksheets["Intern"] != null;
                    if (sheetExists)
                    {
                        try
                        {
                            ...
                            using (var package = new ExcelPackage(new FileInfo(filePath)))
                            {
                                ...
                                using (var outputPackage = new ExcelPackage())
                                {
                                    ...
                                    {
                                        var ctcsheet = CTCPackage.Workbook.Worksheets["Intern"];
                                    ...
                                    for (int row = 2; row <= lastRow2; row++)
                                    {
                                        ...
                                        bool intern = Regex.IsMatch(joinerandchangessheet.Cells[row, designation].Text.ToLower(), $@"\b{Regex.Escape("intern")}\b", RegexOptions.IgnoreCase);
                                        
                                        if (!string.IsNullOrEmpty(bgColor.Rgb) && !bgColor.Rgb.Equals("FFFFFF") && intern)
                                        {
                                        ...
                                        else if (!string.IsNullOrEmpty(bgColor.Rgb) && !bgColor.Rgb.Equals("FFFFFF") && filePath.ToLower().Contains("celonis") && joinerandchangessheet.Cells[row, empgrade].Text.ToLower().Contains("intern"))
                                        {
                                            HRID.Add(joinerandchangessheet.Cells[row, hrid2].GetValue<string>());
                                            PtLoc.Add(joinerandchangessheet.Cells[row, hrid2].GetValue<string>(), Service1.ShrinkString(joinerandchangessheet.Cells[row, ptloc].GetValue<string>()));
                                            Role.Add(joinerandchangessheet.Cells[row, hrid2].GetValue<string>(), Service1.ShrinkString(joinerandchangessheet.Cells[row, designation].GetValue<string>()));
                                    ...
                                    string newFileName = Path.Combine(destinationFolder, Service1.FileCount + "]NEW_Interns_CTC" + Path.GetFileName(filePath));
